...
class netzero(util.Strategy):
  ...
  def makeCombo(self, underlying, current_date, expiration, position_size):
    ...
    lowerlongstrike = util.connector.select_strike_by_delta(
        current_date, underlying, expiration, "p", -20)
    if (lowerlongstrike is None):
      print("lowerlongstrike is None")
      return None
    lowerlongposition = util.makePosition(
        current_date, underlying, lowerlongstrike, expiration, "p", position_size)

    # 2x short put delta 40
    shortstrike = util.connector.select_strike_by_delta(
        current_date, underlying, expiration, "p", -40)
    if (shortstrike is None):
      print("shortstrike is None")
      return None
    shortposition = util.makePosition(
        current_date,  underlying, shortstrike, expiration, "p", -2 * position_size)

    # 1x long put delta 60
    upperlongstrike = util.connector.select_strike_by_delta(
        current_date, underlying, expiration, "p", -60)
    if (upperlongstrike is None):
      print("upperlongstrike is None")
      return None
    upperlongposition = util.makePosition(
        current_date, underlying, upperlongstrike, expiration, "p", position_size)

    combo = util.PutButterfly(
        upperlongposition, shortposition, lowerlongposition)
    return combo
  ...
  def checkExit(self, underlying, combo, dte, current_pnl, max_risk, entry_price, current_date, expiration, dit, position_size):
    ...
    if (self.pct_exit is not None):
      ...
      current_shortdelta = util.connector.select_delta(
          current_date, underlying, expiration, combo.shortposition.option.type, combo.shortposition.option.strike)
