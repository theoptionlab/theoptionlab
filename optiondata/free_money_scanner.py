...
query = "SELECT DISTINCT underlying_symbol FROM fullday WHERE quote_date = '" + str(date) + "' ORDER BY underlying_symbol ASC"
...
for row in cur1.fetchall(): 
    ...
    try: 
        query = "SELECT DISTINCT strike, expiration, (select ((bid_1545+ask_1545)/2) AS call_mid from fullday where option_type='c' AND id=outer_table.id AND bid_1545 != 0 AND ask_1545 != 0 AND ABS(bid_1545 - ask_1545) < 0.5), (select ((bid_1545+ask_1545)/2) AS put_mid from fullday WHERE bid_1545 != 0 AND ask_1545 != 0 AND ABS(bid_1545 - ask_1545) < 0.5 AND option_type='p' AND strike=outer_table.strike AND expiration=outer_table.expiration AND underlying_symbol = outer_table.underlying_symbol AND quote_date = outer_table.quote_date) FROM fullday as outer_table WHERE bid_1545 != 0 AND ask_1545 != 0 AND underlying_symbol = '"+underlying+"' AND quote_date = '" + date + "' AND expiration > (quote_date + INTERVAL '60 day') GROUP BY id, expiration, strike"
        cur2.execute(query)
    except:
        db.rollback()
        continue 
