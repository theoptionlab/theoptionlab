...
def insert(date):
  start = time.time()
  datafilepath = settings.data_dir + \
      "UnderlyingOptionsEODQuotes_" + str(date) + ".zip"
  unzippedpath = util.unzip(datafilepath)

  db = psycopg2.connect(
      "host=localhost dbname=optiondata user=" + settings.db_username)
  ...
  print("CREATE INDEX...")
  cur.execute(
      "CREATE INDEX idx_underlying_symbol_fullday ON fullday USING btree (underlying_symbol)")
  cur.execute(
      "CREATE INDEX idx_strike_fullday ON fullday USING btree (strike)")
  cur.execute(
      "CREATE INDEX idx_expiration_fullday ON fullday USING btree (expiration)")
  cur.execute(
      "CREATE INDEX idx_type_fullday ON fullday USING btree (option_type)")
  cur.execute(
      "CREATE INDEX idx_bid_1545_fullday ON fullday USING btree (bid_1545)")
  cur.execute(
      "CREATE INDEX idx_ask_1545_fullday ON fullday USING btree (ask_1545)")

  print("copy_from and commit")
  with open(unzippedpath, 'r') as f:
    next(f)  # Skip the header row.
    cur.copy_from(f, 'fullday', sep=',', columns=('underlying_symbol', 'quote_date', 'root', 'expiration', 'strike', 'option_type', 'open', 'high', 'low', 'close', 'trade_volume', 'bid_size_1545', 'bid_1545',
                  'ask_size_1545', 'ask_1545', 'underlying_bid_1545', 'underlying_ask_1545', 'bid_size_eod', 'bid_eod', 'ask_size_eod', 'ask_eod', 'underlying_bid_eod', 'underlying_ask_eod', 'vwap', 'open_interest', 'delivery_code'))
    db.commit()
  ...
  cur.execute(
      "UPDATE fullday SET underlying_mid_1545 = (underlying_bid_1545 + underlying_ask_1545) / 2")
  db.commit()
