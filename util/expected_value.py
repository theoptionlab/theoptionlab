...
def getExpectedValue(underlying, combo, current_date, expiration, use_precomputed=True, include_riskfree=True):
  ...
  if use_precomputed:
    try:
      atm_iv = float(util.connector.select_iv(
          current_date, underlying, expiration, "p", atm_strike))
    except:
      atm_iv = 0.01
    ...
    try:
      atm_option = util.Option(
          current_date, underlying, atm_strike, expiration, "p")
    except ValueError:
      return None
    ...
    try:
      atm_iv = float(implied_volatility.implied_volatility(
          midprice, current_quote, atm_strike, remaining_time_in_years, rf, atm_option.type))
    except:
      atm_iv = 0.01
  ...
  one_sd = (atm_iv / math.sqrt(util.yeartradingdays /
  ...
  for i in range(25):  # war 1001
    ...
    for position in positions:
      ...
      value = black_scholes.black_scholes(
          position.option.type, ul_for_ew[i], position.option.strike, remaining_time_in_years, rf, 0)
    ...
    prob = util.prob_hit(
        current_quote, ul_for_ew[i], remaining_time_in_years, 0, atm_iv)
...
def getExpectedValueGroup(underlying, group, current_date, expiration):
  ...
  for combo in combos:
    expected_value += getExpectedValue(underlying,
                                       combo, current_date, expiration)
  return expected_value
