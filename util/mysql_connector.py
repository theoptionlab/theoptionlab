...
class MyDB(object):
    ...
    def __init__(self):
        self._db_connection = pymysql.connect(host="localhost", user=settings.db_username, passwd=settings.db_password, db="optiondata") 
    ...
    def select_expiration(self, quote_date, underlying_symbol, option_type, days): 
        query = "SELECT DISTINCT expiration, datediff(expiration, '" + str(quote_date) + "') AS difference FROM optiondata WHERE underlying_symbol = '" + underlying_symbol + "' AND quote_date = '" + str(quote_date) + "' AND option_type = '" + option_type + "' AND expiration >= '" + str(quote_date) + "' AND expiration <= DATE_ADD('" + str(quote_date) + "', INTERVAL " + str(days + 50) + " DAY)" + " AND expiration >= DATE_ADD('" + str(quote_date) + "', INTERVAL " + str(days - 50) + " DAY) ORDER BY ABS(difference - " + str(days) + ") ASC LIMIT 1;"
        self.query(query)
        row = self._db_cur.fetchone()
        if row == None:
            return None
    ...
    def query_expiration_midprice_before(self, underlying_symbol, strike, quote_date, later_expiration, budget):
        query = "SELECT expiration FROM optiondata WHERE quote_date = '" + str(quote_date) + "' AND underlying_symbol = '" + underlying_symbol + "' AND expiration > '" + str(quote_date) + "' AND expiration < '" + str(later_expiration) + "' AND strike = '" + str(strike) + "' AND mid_1545 <= " + str(budget) + " AND mid_1545 != 0 ORDER BY expiration DESC LIMIT 1"
        self.query(query)
        row = self._db_cur.fetchone()
        if row is None: 
            return None, None
